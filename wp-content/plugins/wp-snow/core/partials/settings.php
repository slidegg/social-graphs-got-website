...
if( isset( $_POST['wpsnow_update_settings'] ) && $_POST['wpsnow_update_settings'] === 'yes' ) {
	...
	foreach( $this->settings as $settings_name => $setting ){
		...
		if( $settings_name === 'wps_flakes_styles' ){
		    if( $value !== '' && ! preg_match( "/^[a-zA-Z0-9\,\#\:\;\ \-\(\)\%\"\_\.\/\!\?]*$/", $value ) ){
			    $value = '';
			    echo '<div class="notice notice-warning"><p>' . __( 'Error while saving the following field. Please make sure you use the correct syntax. Only valid CSS is allowed: ', WPSNOW_TEXTDOMAIN ) . $setting['label'] . '</p></div>';
            }
        }
		...
		if( $settings_name === 'wps_flakes_color' ){
		    if( $value !== '' && ! preg_match( "/^[a-zA-Z0-9\,\#\ ]*$/", $value ) ){
			    var_dump($value);
			    $value = '';
			    echo '<div class="notice notice-warning"><p>' . __( 'Error while saving the following field. Please make sure you use the correct syntax. Only hex colors and "," are allowed: ', WPSNOW_TEXTDOMAIN ) . $setting['label'] . '</p></div>';
            }
        }
		...
		if( $settings_name === 'wps_show_on_specific_pages_only' ){
		    if( $value !== '' && ! preg_match( "/^[0-9\,\ ]*$/", $value ) ){
			    $value = '';
			    echo '<div class="notice notice-warning"><p>' . __( 'Error while saving the following field. Please make sure you use the correct syntax. Only numbers and "," are allowedx: ', WPSNOW_TEXTDOMAIN ) . $setting['label'] . '</p></div>';
            }
        }
		...
		if( $settings_name === 'wps_flakes_font' ){
		    if( $value !== '' && ! preg_match( "/^[a-zA-Z0-9\,\-\ ]*$/", $value ) ){
			    $value = '';
			    echo '<div class="notice notice-warning"><p>' . __( 'Error while saving the following field. Please make sure you use the correct syntax. Only numbers, letters and "," "-" are allowed: ', WPSNOW_TEXTDOMAIN ) . $setting['label'] . '</p></div>';
            }
        }
	...
	echo '<div class="notice notice-success"><p>' . __( 'Settings saved.', WPSNOW_TEXTDOMAIN ) . '</p></div>';
}

?>
<h2><?php echo WPSNOW_NAME; ?></h2>
<p>
	<?php _e( 'On this page, you will find all available settings for your snow flakes.', WPSNOW_TEXTDOMAIN ); ?>
</p>
...
<form method="post" action="">
				...
				<td>
					...
					<?php else : ?>
						...
						<?php if( ! empty( $value ) && $value !== 'empty' ) : ?>
							<p class="fa-preview"><?php echo __( 'Preview:', WPSNOW_TEXTDOMAIN ); ?> <?php echo "<i style='font-size:18px;' class='fas fa-" . sanitize_title( $value ) . "'></i>"; ?></p>
						<?php endif; ?>
					<?php endif; ?>
					<p class="description">
	...
	<?php submit_button( __( 'Save', WPSNOW_TEXTDOMAIN ) ); ?>
