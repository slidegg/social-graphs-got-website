...
if ( ! class_exists( 'WP_Snow' ) ) :
	...
	class WP_Snow{
		...
		public function add_user_submenu(){
			add_submenu_page( 'options-general.php', __( 'WP Snow', WPSNOW_TEXTDOMAIN ), __( 'WP Snow', WPSNOW_TEXTDOMAIN ), $this->get_admin_cap(), sanitize_title( WPSNOW_NAME ), array( $this, 'render_admin_submenu_page' ) );
		}
		...
		public function render_admin_submenu_page(){
			if( ! current_user_can( $this->get_admin_cap() ) ){
				wp_die( __( 'Sorry, but you don\'t have enough permission.', WPSNOW_TEXTDOMAIN ) );
			}
		...
		private function load_settings(){
			...
			$fields = array(
				...
				'wps_flakes_deactivate' => array(
					...
					'type'        => 'checkbox',
					'label'       => __( 'Deactivate Snowflakes', WPSNOW_TEXTDOMAIN ),
					'placeholder' => '',
					'required'    => false,
					'description' => __( 'Check this button in case you want to temporarily deactivate the snow flakes.', WPSNOW_TEXTDOMAIN )
				),

				'wps_flakes_number' => array(
					'id'          => 'wps_flakes_number',
					'type'        => 'number',
					'label'       => __( 'Number of flakes', WPSNOW_TEXTDOMAIN ),
					'placeholder' => 45,
					'required'    => false,
					'description' => __( 'This is the total number of flakes that should be visible on one page at once.', WPSNOW_TEXTDOMAIN )
				),

				'wps_flakes_falling_speed' => array(
					'id'          => 'wps_flakes_falling_speed',
					'type'        => 'text',
					'label'       => __( 'Flakes falling speed', WPSNOW_TEXTDOMAIN ),
					'placeholder' => 0.5,
					'required'    => false,
					'description' => __( 'The falling speed of the flakes. Please include a value like 0.1 or 5.0 - the higher the value, the faster they will fall.', WPSNOW_TEXTDOMAIN )
				),

				'wps_flakes_max_size' => array(
					'id'          => 'wps_flakes_max_size',
					'type'        => 'number',
					'label'       => __( 'Flakes maximum size', WPSNOW_TEXTDOMAIN ),
					'placeholder' => 30,
					'required'    => false,
					'description' => __( 'Please include a number for the maximum size of your flakes. The higher the bigger.', WPSNOW_TEXTDOMAIN )
				),

				'wps_flakes_min_size' => array(
					'id'          => 'wps_flakes_min_size',
					'type'        => 'number',
					'label'       => __( 'Flakes minimum size', WPSNOW_TEXTDOMAIN ),
					'placeholder' => 12,
					'required'    => false,
					'description' => __( 'Please include a number for the minimum size of your flakes. The lower the smaller.', WPSNOW_TEXTDOMAIN )
				),

				'wps_flakes_refresh_time' => array(
					'id'          => 'wps_flakes_refresh_time',
					'type'        => 'number',
					'label'       => __( 'Flakes refresh time', WPSNOW_TEXTDOMAIN ),
					'placeholder' => 50,
					'required'    => false,
					'description' => __( 'Please include a number for the refresh time of your flakes. This is the time it takes to make the next move. Measured in milliseconds.', WPSNOW_TEXTDOMAIN )
				),

				'wps_flakes_z_index' => array(
					'id'          => 'wps_flakes_z_index',
					'type'        => 'number',
					'label'       => __( 'Flakes z-index', WPSNOW_TEXTDOMAIN ),
					'placeholder' => 50,
					'required'    => false,
					'description' => __( 'The z-index determines the depth of your flakes. the higher the number, the more likely it is that they are placed over all available elements.', WPSNOW_TEXTDOMAIN )
				),

				'wps_flakes_entity' => array(
					'id'          => 'wps_flakes_entity',
					'type'        => 'text',
					'label'       => __( 'Flakes Entity', WPSNOW_TEXTDOMAIN ),
					'placeholder' => "*",
					'required'    => false,
					'description' => __( 'This is what will be shown as a flake. Please note, that special characters are sanitized so that they will be visible within the frontend. Double qotes are replaced with single quotes.', WPSNOW_TEXTDOMAIN )
				),

				'wps_flakes_styles' => array(
					'id'          => 'wps_flakes_styles',
					'type'        => 'text',
					'label'       => __( 'Additional Flake Styles', WPSNOW_TEXTDOMAIN ),
					'placeholder' => "color:green;border:1px solid #000;",
					'required'    => false,
					'description' => __( 'In here you can add custom styles you want to attach to every snow flake.', WPSNOW_TEXTDOMAIN )
				),

				'wps_flakes_color' => array(
					'id'          => 'wps_flakes_color',
					'type'        => 'text',
					'label'       => __( 'Flakes color', WPSNOW_TEXTDOMAIN ),
					'placeholder' => "#aaaacc,#ddddff,#ccccdd,#f3f3f3,#f0ffff",
					'required'    => false,
					'description' => __( 'You can add multiple colors for your flakes with a comma separated. You can pick your colors easily on <a target="_blank" href="https://www.color-hex.com/">https://www.color-hex.com/</a>', WPSNOW_TEXTDOMAIN )
				),

				'wps_flakes_font' => array(
					'id'          => 'wps_flakes_font',
					'type'        => 'text',
					'label'       => __( 'Flakes Fonts', WPSNOW_TEXTDOMAIN ),
					'placeholder' => "Times,Arial,Times,Verdana",
					'required'    => false,
					'description' => __( 'You can add multiple fonts for your flakes with a comma separated. These fonts will affect how your snowflake looks.', WPSNOW_TEXTDOMAIN )
				),

				'wps_activate_font_awesome' => array(
					'id'          => 'wps_activate_font_awesome',
					'type'        => 'checkbox',
					'label'       => __( 'Activate Font Awesome', WPSNOW_TEXTDOMAIN ),
					'placeholder' => "",
					'required'    => false,
					'description' => __( 'This allows you to add font awesome icons as your falling snow flakes. The additional settings appear once you click the save button.', WPSNOW_TEXTDOMAIN )
				),

				'wps_choose_fa_icon' => array(
					'id'          => 'wps_choose_fa_icon',
					'type'        => 'select',
					'label'       => __( 'Choose Fontawesome Icon', WPSNOW_TEXTDOMAIN ),
					'choices'     => $fa_icons,
					'disabled'    => ( get_option( 'wps_activate_font_awesome' ) === 'yes' ) ? false : true,
					'placeholder' => "",
					'required'    => false,
					'description' => __( 'If you choose an icon from this list, the default snowflake entity will be overwritten.', WPSNOW_TEXTDOMAIN )
				),

				'wps_show_on_specific_pages_only' => array(
					'id'          => 'wps_show_on_specific_pages_only',
					'type'        => 'text',
					'label'       => __( 'Show only on specific posts or pages.', WPSNOW_TEXTDOMAIN ),
					'placeholder' => "18,174,2",
					'required'    => false,
					'description' => __( 'Please specify the post ids or page ids on which you want to display the snow. If you set values within this field, the snow doesn\' show up anywhere else except of this posts and pages.', WPSNOW_TEXTDOMAIN )
				),
